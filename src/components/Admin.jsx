...
const Admin = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3005/reports')
            .then(res => setUpgradeRoleData(res.data))
            .catch(err => console.log(err));
    }, []);
    return (
        ...
        border-t-[1px] border-gray-100 drop-shadow-md">
                ...
                {stage === 2 && boardsData &&
                    <div className="px-10 pt-6">
                        <div className="w-full grid grid-cols-12 text-blue-800">
                            <p className="col-span-1">อันดับ</p>
                            <p className="col-span-4">หัวข้อ</p>
                            <p className="col-span-2">ผู้เขียน</p>
                            <p className="col-span-2">วันที่ยื่นคำร้อง</p>
                            <p className="col-span-2">ไฟล์</p>
                            <p className="col-span-1">คำสั่ง</p>
                        </div>
                        {upgradeRoleData.map((article, index) => (
                            <div className="w-full grid grid-cols-12 my-2 border-b-[1px] py-1">
                                <div className="col-span-1">{index + 1}</div>
                                <div className="col-span-4 ">
                                    <p>{article.title}</p>
                                ...
                                <div className="col-span-2">
                                    <p className="underline underline-offset-1 text-blue-800 cursor-pointer"
                                    onClick={() => window.open(article.status, '_blank')}>
                                        {article.status.substring(0, 20)}</p>
                                </div>

                                <div className="col-span-1">
                                    <button className="bg-green-200 w-full py-1 rounded-md">อนุมัติ</button>
                                </div>
                            </div>
                        ))}
